...
// class for functions simulating Page Replacement Algorithms and associated
// data
class PageReplacement {
  private:
    ...
    vector<string>
        fault_list;           // to store whether each access was a fault or not
    ...
    void FIFO_and_LRU(
        string replacement_algorithm); // to simulate FIFO and LRU algorithms
    void OPTIMAL();                    // to simulate OPTIMAL algorithm
    void display(); // to display which page is in each frame after each access
  public:
    PageReplacement(string input_file_name,
                    string input_roll_number);   // to initialize values
    void evaluate(string replacement_algorithm); // to simulate the algorithm
};

/*
 * constructor function
 * assigns values to const variables, opens files, and initializes vectors and
 * ints
 */
PageReplacement::PageReplacement(string input_file_name,
                                 string input_roll_number)
    ...
    {
        cout << "Error : Number of frames must be a non negative integer"
             << endl;
        exit(1);
    }
...
void PageReplacement::evaluate(string replacement_algorithm) {
    if (replacement_algorithm == "FIFO" ||
        replacement_algorithm == "LRU") // call based on algorithm specified
    {
    ...
    output_file.open(
        (roll_number + "_" + replacement_algorithm + ".out").c_str(),
        ios::out);    // open output file
...
void PageReplacement::FIFO_and_LRU(string replacement_algorithm) {
    ...
    {
        it = find(frames.begin(), frames.end(),
                  current_page); // check if already in a frame
        if (it != frames.end())  // if already there
        {
            ...
            if (replacement_algorithm == "LRU") {
                ...
                ordering[index] = 0; // if the algorithm is LRU, access resets
                                     // the order so that it is last in the list
            }
        } else // if not there
        {
            if (number_of_frames_used <
                number_of_frames) // if an empty frame is available
            {
                frames[number_of_frames_used] = current_page; // add frame
                number_of_frames_used++; // indicate that one more frame has
                                         // been used
            } else                       // if no empty frame is available
            {
                int evict_index = distance(
                    ordering.begin(),
                    max_element(ordering.begin(),
                                ordering.end())); // the one to be evicted is
                                                  // the one whose ordering value
                                                  // is the highest
                if (evict_index <
                    number_of_frames) // to check if index is in range
                {
                    frames[evict_index] =
                        current_page;          // replace with the current page
...
void PageReplacement::OPTIMAL() {
    int current_page;         // the current page being accessed
    vector<int>::iterator it; // to iterate over the frames
    vector<int> marker; // to mark the index of the next page access of the page
                        // at the given frame
    ...
    for (int i = 0; i < page_accesses.size(); i++) {
        ...
        it = find(frames.begin(), frames.end(),
                  current_page); // check if it is already in a frame
        if (it != frames.end())  // if already there
        {
            fault_list.push_back(
                no_fault); // indicate that this access has no fault
        } else             // if not there
        {
            if (number_of_frames_used <
                number_of_frames) // if there is an empty frame
            {
                frames[number_of_frames_used] =
                    current_page;        // store in the next available frame
            ...
            } else                       // if there is no empty frame
            {
                ...
                for (int j = 0; j < frames.size(); j++) {
                    if (is_set[j] == false || marker[j] <= i) // if marker for
                                                              // current frame
                                                              // needs to be
                                                              // updated
                    {
                        find_it = find(page_accesses.begin() + i + 1,
                                       page_accesses.end(),
                                       frames[j]); // find earliest next access
                                                   // of page in current frame
                        marker[j] = distance(
                            page_accesses.begin(),
                            find_it); // store index of access in marker
                        is_set[j] =
                            true; // marker does not need to be updated for now
                    }
                }
                int evict_index =
                    distance(marker.begin(),
                             max_element(marker.begin(),
                                         marker.end())); // the highest value in
                                                         // marker is the latest
                                                         // accessed page, the
                                                         // one to be evicted
                if (evict_index <
                    number_of_frames) // to check if index is in range
                {
                    frames[evict_index] =
                        current_page; // replace with the current page
                    is_set[evict_index] =
                        false; // marker needs to be updated next iteration
                }
            }
...
int main(int argc, char *argv[]) {
    ...
    PageReplacement pageReplacement(
        file_name, roll_number); // create and initialize object
